...
// (c) 2006-2017 Andre Massow britnex@gmail.com ,    
...
Ad.prototype.init = function(){
	...
	var self = this;
	if(this.gad && this.gad.length>0) 
	{
		if(window.doads === undefined) 
		{
			self.gad.load("/"+self.gad.attr("id")+"/");
			try {
			  ga('send', 'event', 'ads', 'load', 'adblock');
			}catch (e) {}
		}
	}
...
function Graph(){
	...
	this.trigodeg = null;
	this.trigorad = null;
	this.loadurl = "/js-tmpl/graph.html";
	this.type = "graph";
	this.minrowcount = 2;
...
Graph.prototype.show = function(){
	...
	div.load( self.loadurl, function(){
		...
		self.modal.addClass("type-"+self.type);
         ...
         success: function (json) {
        	 ...
        	 for (var i=0;i<results.length && i<rows.length;i++)
    		 {
        		 ...
        		 switch (r.status) {
        		 	...
        		 	case "overloaderror":
        		 		resultimg.attr("src","img/empty.png");	 
        		 		alert("Too many requests from your ip. Please wait 1 minute.");
        			 break;
...
function Sketch() {
	this.loadurl = "/js-tmpl/sketch.html?cb="+Math.random();	
	this.type = "sketch";
	...
	this.axisbounds = undefined; //[-10,10,-10,10];
}
	...
	var self = this;
	 ...
	 var edit = self.modal.find("div.edit");
	 	...
	 	edit.children("ul.rows").children("li.row").each(function(){
	 		var row = $(this);
	 		var id = row.attr("id");
	 		var label = row.find("select.labelstyle").val();
		...
		if ((json) || (('localStorage' in window && window['localStorage'] !== null))) {		    				
		    if (localStorage || json) { 	
		    	...
		    	if (json) {
		    		...
		    		var sketchids = self.modal.find("#sketchids");
		    		sketchids.empty();
		    		...
		    		self.modal.find("div.edit").children("ul.rows").children("li.row").each(function(){
		    	 		...
		    	 		var field = fields[i];
		    	 		if (field) {
		    	 			var typeselect = row.find("select.type:first");
			    	 		typeselect.val(field.selected);
			    	 		changeDrawType( typeselect );
			    	 		
			    	 		row.attr("id", field.id );
			    	 		row.find("select.labelstyle").val( field.id );
			    	 		row.find("span.line.color").text( field.colors[0] );
			    	 		row.find("span.color.background").text( field.colors[1] );
		    		...
		    		sketchids.find("option:first").attr("selected","selected");
...
Sketch.prototype.prepare = function(div){
	...
	var f = graphdiv.height()>0?(1.0*graphdiv.width()/graphdiv.height()):1;
	//if (!self.axisbounds)
	self.axisbounds = [-10,10,2*-10*f,2*10*f];
	...
	var zoomctl = modal.find(".zoomctl button.btn-sm, .zoomctl a.btn-sm");
	 var btn_reset = zoomctl.parent().find(".reset");
	  ...
	  var btn_zoomin = zoomctl.parent().find(".zoomin");
	  var btn_zoomout = zoomctl.parent().find(".zoomout");
	 ...
	 var btn_delete = modal.find(".delete");
	 btn_delete.unbind("click");
	 btn_delete.click(function(event){
	 	var sketchids = modal.find("#sketchids");
	 	var options = sketchids.find("option");
	 	if (options.length>1) {
	 		var selected = sketchids.val();
	 		sketchids.find("option[value="+selected+"]").remove();
	 		modal.find("ul.rows li[id="+selected+"]").remove();
	 		modal.find("ul.rows li").removeClass("active");
	 		modal.find("ul.rows li:first").addClass("active");
	 		modal.find("option:first").attr("selected","selected");
	 		self.refresh();		  
		  	event.stopPropagation();    
	 	}
	 });
	 ...
	 modal.find(".save").click(function(){
	 	...
	 	var name = encodeURIComponent( self.modal.find("input.name").val() );
	 	$.ajax({
         url: "/js-tmpl/sketch.html?save="+self.id,
         dataType: 'json',
         data: {"json":self.savejson, "name": name},
         async: true,
         type: 'POST',
         ...
         success: function (json) {
         	...
         	if (json) {
         		...
         		}else
         		{
				 	...
				 	var link = modal.find("a.link");
				 	link.parent().show();
				 	var href = "/sketches/"+self.id+".png";
				 	link.attr("href", href );
				 	link.find("ins").html((window.location.href+"/").substring(0, window.location.href.indexOf("/", 7)) +href);
			 	}
		 	}
	 ...
	 var sketchids = modal.find("#sketchids");
	 sketchids.bind("mouseup keyup", function(){
	 
	 	
	 	
	 	var selected = sketchids.val();
	 	
	 	modal.find("ul.rows li").each(function(){
	 		var li = $(this);
	 		if (li.attr("id")==selected)
	 			li.addClass("active");
	 		else
	 			li.removeClass("active");
	 	});
...
Sketch.prototype.addRow = function(){
	...
	firstselect.data("lasttype","-");
	
		
	li.find("input[name='result']").val("");
	li.find(".jPicker").remove();
	
	var name = rowtype+""+ (self.rowcount+1);
	
	li.attr("id", name );
	li.data("index", self.rowcount+1 );
	
	$("#sketchids").append($('<option>', {
	    value: name,
	    text: name
	}));
	$("#sketchids option:last").attr("selected","selected");
	li.find("input.lbl").val(name);
...
function selectsubtype( select ){
	...
	select.parents("li:first").children('ul.active').removeClass("active");
	select.parents("li:first").children('ul.'+select.val()).addClass("active");
}

function changesketchitemname(inp) {
	var li = inp.parents("li:first");
	
	var oldname = li.attr("id");
	var newname = inp.val();
	...
	else
	{
		...
		if (newname.length==0 || !reg.test(newname)) // invalid name
		{
		...
		var option = $("#sketchids option[value="+newname+"]");
		if (option.length>0) // not available
		{
			alert("name not available");
			newname = oldname;
			inp.val(newname);
		...
		option = $("#sketchids option[value="+oldname+"]");
		 option.val(newname);
		 option.text(newname);
	...
	inp.parents("div.edit:first").find("li.row").each(function(){
		var row = $(this);
		row.find("select").each(function(){
			var select = $(this);
			select.find("option").each(function(){
				var option = $(this);
				if (option.val()==oldname) {
					option.val(newname);
					option.text(newname);
				}
			});
...
function changeDrawType(select){
	...
	var typechanged = (select.data("lasttype")!=type);
	select.data("lasttype", type);
	...
	if (typechanged) {
		var index = select.parent().data("index");
		var oldname = id;
		var newname = type+""+index;
		
		switch(type) {
			case "circle": newname="C"+index; break;
			case "line": newname="L"+index; break;
			case "straight": newname="S"+index; break;
			case "point": 
				if (index < 26)
				newname=String.fromCharCode(64+index); 
				else
				newname="P"+index; 
			break;
		}
	
		select.parent().attr("id", newname);
		div.find(".lbl").val(newname);
		var option = $("#sketchids option[value="+oldname+"]");
		option.val(newname);
		option.text(newname);
	}
			...
			if (arr!=null) {
				...
				for (var i=0;i<arr.length;i++){
					...
					//	select.append($("<option></option>").attr("value",id).text(arr[i][0]+((arr[i][1].length>0)?(": "+arr[i][1]):"") ));			
						select.append($("<option></option>").attr("value",id).text( arr[i][1] ));			
				};
...
Sketch.prototype.loaddeps = function() {
	...
	$.getScript("/js/jpicker-1.1.6.min.js",
	...
	$("<link/>",{rel:"stylesheet",type:"text/css",href:"/"+"css/jpicker-1.1.6min.css"}).appendTo("head");
...
var self = this;
	...
	self.modal.find("div.edit ul li.row").each(function(index){
		...
		var colors = li.find("span.color.line").text()+","+li.find("span.color.background").text();
		var label = li.find("select.labelstyle").val();
		if (label)
			label = id.replace("\"",""); //$.trim(label.replace("\"","'"));
		else
			label = " ";
		...
		//styles.push( JSON.stringify({ "lc":li.find("span.color.line").text(), "bg":li.find("span.color.background").text(), "lbl":label }) );
		
		style= ", {\""+label+"\", "+li.find("span.color.line").text()+", "+li.find("span.color.background").text()+"}";
...
//	var parameters = {"in": rows, "trig":"rad", "sketch":1, "p":displayoptions.precision, "s":displayoptions.scientific, "styles":(styles) };
	...
	*/
	...
	self.savejson =JSON.stringify( parameters );
	...
	$.ajax({
         ...
         success: function (json) {
        	 ...
        	 var results = json.results;
	        	...
	        	self.modal.find(".save").prop("disabled", false);	        		
...
function initLatexFormulas() {
	...
	//if ($("#editor").length==0) 
	{
		...
		$("<script type=\"text/x-mathjax-config\">MathJax.Hub.Config({ tex2jax: {preview: \"none\"}, jax: [\"input/TeX\",\"output/SVG\"], skipStartupTypeset:true, TeX: { extensions: [ \"color.js\"] }});MathJax.Hub.Register.StartupHook('End',function () { postInitLatexFormulas(); });</script>").appendTo($("head"));
